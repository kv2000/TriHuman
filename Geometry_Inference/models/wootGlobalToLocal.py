...
class wootGlobalToLocal(nn.Module):
    def __init__(
            ...
            _jelly_offset = 0.049, device = 'cuda:0', max_sample_num = 5760001
        ):
        super().__init__()
    ...
    def compute_closet_faces(self, t_query_xyz, temp_verts, cand_ids):
        ...
        is_type5 = (ubc >= 0.) & (ubc <= 1.) & is_not_above(v1, e21, fN, query_xyz_expanded)
        is_type6 = (uca >= 0.) & (uca <= 1.) & is_not_above(v2, e02, fN, query_xyz_expanded)
        ...
        type_0_dist = compute_dist(
            query_xyz_expanded, type_0_pts
        )

        # type 1, 2, 3 map to vertices, no direction
        type_1_dist = compute_dist(query_xyz_expanded, v0)
        type_2_dist = compute_dist(query_xyz_expanded, v1)
        type_3_dist = compute_dist(query_xyz_expanded, v2)
        ...
        # type 4, 5, 6 dist
        type_4_dist = compute_dist(query_xyz_expanded, type_4_pts)
        type_5_dist = compute_dist(query_xyz_expanded, type_5_pts)
        type_6_dist = compute_dist(query_xyz_expanded, type_6_pts)
        ...
        min_idx = torch.argmin(merged_distance, dim=-1)
