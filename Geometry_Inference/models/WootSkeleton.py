...
@Date: 2023-09-25
@Desc: The pytorch skeleton, which converts the dofs to the joint translations/transformations
"""
...
import kornia
from pytorch3d.transforms import axis_angle_to_matrix as angle_axis_to_rotation_matrix
...
class WootSkeleton(nn.Module):
    ...
    def compute_layered_parent_id(self, org_parent_id):
        ...
        print('num layers:', len(ret_cur_idx))
        print(ret_cur_idx,' ',ret_fa_idx)
    ...
    def forward(self, dof):
        ...
        revolute_mat = angle_axis_to_rotation_matrix(revolute_vec)
...
if __name__ == '__main__':
    ...
    dof_dir = '/CT/wootidd0/work/projects/woot_deep_characters_clean/test_data/Subject0028_loose_0/hand_charactor/hand_charactor.motion'
    #dof_dir = 'D:/hemingzhu/woot_deep_characters_clean/test_data/Subject0028_loose_0/skeletoolToGTPose/poseAngles.motion'

    dof_arr = CSVHelper.load_csv_sequence_2D(
        dof_dir, type='float', skipRows=1, skipColumns=1, end_frame=(10000)
    )

    temp_skeleton = WootSkeleton(
        #skeleton_dir= 'D:/hemingzhu/woot_deep_characters_clean/test_data/Subject0028_loose_0/ddc.skeleton',
        skeleton_dir= '/CT/wootidd0/work/projects/woot_deep_characters_clean/test_data/Subject0028_loose_0/hand_charactor/actor.skeleton',
        device = 'cuda'
    )

    dof_arr = torch.FloatTensor(dof_arr).cuda()

    sum_time = 0.
    sum_time_1 = 0
    import time
    
    ret = temp_skeleton.forward(dof_arr[0][None,...])

    torch.cuda.synchronize()
    st = time.time()
    
    for i in tqdm(range(8000,8001)):
        ret_joint_transformation, ret_local_joint_translation, ret_local_joint_transformation = temp_skeleton.forward(dof_arr[[i%10000,((i+1)%10000),((i+2)%10000)]])
