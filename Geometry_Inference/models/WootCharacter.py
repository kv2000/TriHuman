...
@Date: 2023-09-25
@Desc: The pytorch learnable embedded graph charactor, support for the charactor with/without hands.
"""
...
import trimesh
from models.WootSkeleton import WootSkeleton
from models.character_utils import dual_quad_to_trans_vec, batch_mat_to_angle, wrap_transformation_angle, compute_geodesic_distance, compute_trans_quad, batch_angle_to_mat

from einops import rearrange

def compute_laplacian(
    deformed_verts = None, sparse_lap = None
):

    batch_size = deformed_verts.shape[0]
    num_verts = deformed_verts.shape[1]

    deformed_vec = deformed_verts.transpose(0, 1).reshape([num_verts, -1])
    lap_deformed = torch.sparse.mm(sparse_lap, deformed_vec)
    lap_deformed = lap_deformed.reshape([num_verts, batch_size, 3]).transpose(0, 1)
...
class WootCharacter(nn.Module):
    def __init__(
            ...
            hand_mask_dir = None, # also used for masking out the nonrigid deformations,
            use_sparse = True
        ):
    ...
    def load_template_mesh(self, mesh_dir):
        ...
        self.temp_faces = torch.LongTensor(self.obj_reader.facesVertexId).reshape([-1, 3]).to(self.device).detach()
        self.temp_vert_colors = torch.FloatTensor(self.obj_reader.vertexColors).reshape([-1, 3]).to(self.device).detach()
        self.temp_uv_coords = torch.FloatTensor(self.obj_reader.textureCoordinates).reshape([-1,3,2]).to(self.device).detach()
        self.temp_verts = torch.FloatTensor(self.obj_reader.vertexCoordinates).reshape([-1,3]).to(self.device).detach()
    ...
    def build_embedded_graph(self, graph_dir): 
        ...
        # compute the highest and lowest for graph
    ...
    def dqs_blending(self, translation_mat):
        ...
        # then compute scale for normalizaton
        raw_scale = torch.norm(
            weighted_rotation, p=2, dim = -1
        )

        # so if te scale is samll enough
    ...
    def forward_test(self, dof = None, delta_R = None, delta_T = None, per_vertex_T = None, cached_global_transform = None):
        ...
        ret_posed_delta = self.compute_embedded_graph_deformation_test(
            blended_mat = blended_mat, 
            deltaR = delta_R, deltaT = delta_T, 
            perVertex_displacement = per_vertex_T
        )

        return ret_posed_delta, cur_global_tranform[:,:,:]
