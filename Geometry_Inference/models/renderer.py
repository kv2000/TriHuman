...
@Date: 2023-09-25
@Desc: A nerual render that adopts the NeuS rendering, but on ITTS space
"""
...
class TrihumanRender(nn.Module):
    ...
    def mapping_wrapper(self, xyz, mesh_vert_pos=None, _padding_height=None):
        ...
        if self.use_uvd:
            fin_uv, uv_distance, _, _, _, _, fin_m_type, fin_proj_pts, fin_fn = self.global_to_local.forward(
                xyz, mesh_vert_pos
            )
    ...
    def forward_org(self, rays_o_, rays_d_, near_, far_, normalized_vert_pos_, cos_anneal_ratio = 0., pose_feature = None, global_translation = None, global_scale = None, is_train = False):       
        z_vals = torch.linspace(0.0, 1.0, self.n_samples + 1).float().to(self.device)
        
        if (is_train):
            jitter_std = 1.0/self.n_samples * 0.3
            ran_val = torch.randn(self.n_samples - 1) * jitter_std
            ran_val = torch.clamp(ran_val, -0.49 * 1.0/(self.n_samples), 0.49 * 1.0/(self.n_samples))
            z_vals[1:-1] += ran_val.float().to(self.device)
        ...
        fin_uvd, fin_mask = self.mapping_wrapper(
            flattend_sample_pts, normalized_mesh_vert_pos
        )
        ...
        global_gradient = global_gradient[:,:3]
        
        global_dir = global_dir.reshape([-1, 3])
        global_dir = global_dir.detach()
            
        # cuz the lights really varies :D
                
        differed_sample_pts = flattend_sample_pts * global_scale + global_translation
        sampled_color = self.color_network(
            differed_sample_pts, global_gradient, global_dir, feature_vector
